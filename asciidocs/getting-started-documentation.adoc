...
You can add pictures just as you would add images to a normal asciidoc file with the exception, that you only need to write the name and not the directories. This will definitely still be marked as an error. To avoid the error you can but the following at the start of the file.
